...
- `tests/` runs on CPU, no GPU, no network. **Two tiers:**
  ...
  **The budget line above is provisional, and this laptop cannot currently settle it
  (2026-08-02).** What is measured, and what is not:

  | run | conditions | result |
  |---|---|---|
  | 663 tests, before the jit-cache fix | cold start, load 1.90 | **463.75 s** |
  | 663 tests, after the fix | load 12.1, package 95 C | 474.62 s |
  | 663 tests, after the fix | load 5.0, package 95 C | 446.97 s |

  Read across that table and the fix looks like it did nothing. It is not that: measured
  back to back under identical conditions, `test_strategies.py` went **225 s to 127 s** and
  `test_coupling.py` plus `test_lowrank.py` went **~110 s to 58 s**. Roughly 150 s came off
  the suite and none of it is visible in the totals, because no post-fix run started cold.

  **Thermal state is a bigger confound than contention on this machine.** The same commit
  measured 464 s cold and 649 s hot: a ~40% penalty. The CPU package sits at 87-97 C against
  a 100 C limit during a full run, so a second full run started immediately after the first is
  throttled for its whole duration. **Let the package drop below ~60 C before comparing two
  timings**, and record the starting temperature next to the number.

  A cooldown wait of 30 minutes failed to get below 87 C, because another agent session was
  running this same suite continuously in the background (a Codex sandbox, 67 minutes and
  counting). **Check `ps` for other people's pytest, not just your own**; `uptime` alone
  attributes their load to nothing.

  So: nothing has been changed on the strength of these numbers, including the budget. The
  honest state is that ~150 s of real saving exists and the total that would show it has not
  been measured. If a cold post-fix run lands under ~6 min, the budget needs no revision at
  all, which is the outcome to check for first.
